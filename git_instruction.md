...
```
Как и `git pull` , `git clone` это составная комманда. Она создает папку на вашем диске, инициализирует в ней новый репозиторий (`git init`), привязывает к нему удаленный репозиторий (`git remote add`), задает ему по умолчанию имя **origin**, скачивает изменения (`git fetch`) и извлекает посkедний коммит (`git checkout`).  
...
## Работа с GitHub
GitHub - это самый популярный онлайн-сервис для совместной работы и хостинга проектов.  
